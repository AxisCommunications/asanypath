...
### S3 (c=1, 10 objects, 3 rounds)

| Operation | asanypath | s3fs | aiobotocore | cloudpathlib |
|-----------|-----------|------|-------------|--------------|
| write | 23.08 ms | 24.95 ms | 32.02 ms | 86.17 ms |
| read | 3.66 ms | 9.39 ms | 22.83 ms | 26.93 ms |
| exists | 3.47 ms | 4.27 ms | 21.63 ms | 3.50 ms |
| iterdir | 45.03 ms | 35.20 ms | 54.74 ms | 32.54 ms |

### Azure (c=1, 10 objects, 3 rounds)

| Operation | asanypath | azure-sdk | adlfs | cloudpathlib |
|-----------|-----------|-----------|-------|--------------|
| write | 2.65 ms | 2.63 ms | 3.01 ms | 12.41 ms |
| read | 1.29 ms | 1.40 ms | 3.25 ms | 4.28 ms |
| exists | 0.86 ms | 1.36 ms | 1.44 ms | 1.58 ms |
| iterdir | 1.32 ms | 2.87 ms | 5.10 ms | 2.97 ms |

### GCS (c=1, 10 objects, 3 rounds)

| Operation | asanypath | gcloud-aio | cloudpathlib | gcsfs |
|-----------|-----------|------------|--------------|-------|
| write | 4.23 ms | 4.41 ms | 13.73 ms | 574.59 ms |
| read | 1.82 ms | 1.49 ms | 5.61 ms | 297.12 ms |
| exists | 2.02 ms | 1.53 ms | 2.07 ms | 2.61 ms |
| iterdir | 0.50 ms | 0.83 ms | 1.33 ms | 278.26 ms |

### Artifactory (c=1, 10 objects, 3 rounds)

| Operation | asanypath | aiohttp | requests |
|-----------|-----------|---------|----------|
| write | 43.53 ms | 26.50 ms | 31.78 ms |
| read | 5.58 ms | 4.01 ms | 4.79 ms |
| exists | 3.82 ms | 2.57 ms | 3.17 ms |
| iterdir | 4.35 ms | 4.00 ms | 4.24 ms |

Latencies are per-operation means. asanypath matches or beats dedicated client
libraries on most operations while providing a single unified API across all
backends.
...
await path.update_access_policy(
    AccessPolicyPatch(provider={"s3_acl": policy.provider["s3_acl"]})
)
```
