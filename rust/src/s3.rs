...
impl CloudBackend for S3Creds {
    ...
    fn put(
        ...
        async move {
            ...
            let s3_path = format!("/{path}");
            let (status, body, _) =
                do_s3_request(self, "PUT", &s3_path, &query, Some(data), use_h2, &headers)
                    .await
                ...
                match name {
                    ...
                    "Grantee" => {
                        ...
                        for attribute in element.attributes().flatten() {
                            if attribute.key.as_ref().ends_with("type") {
                                if let Ok(value) =
                    ...
                    "Permission" if current_grant.is_some() => {
                        field = Some("Permission".to_string())
                    }
                    _ => {}
                }
            }
        ...
        let (type_name, element_name) = match kind {
            ...
            _ => {
                return Err(PyValueError::new_err(format!(
                    "unsupported S3 ACL principal: {kind}"
                )));
            }
        };
        xml.push_str(&format!(
