...
impl CloudBackend for GcsCreds {
    ...
    fn copy(
        ...
        async move {
            let base = self.endpoint.trim_end_matches('/');
            let url = crate::backend::append_query(&format!(
    ...
    fn put(
        ...
        async move {
            ...
            let url = crate::backend::append_query(&append_params(
                &self.upload_base(),
            ...
            let data = if options.provider.is_empty() {
                ...
                let metadata_json = format!(
                    "{{\"name\":{},{}",
                    sonic_rs::to_string(path).map_err(|error| PyValueError::new_err(error.to_string()))?,
                    &provider_json[1..],
                );
...
async fn do_gcs_get_acl(creds: &GcsCreds, object_path: &str) -> Result<String, PyErr> {
    ...
    if !status.is_success() {
        return Err(status_to_pyerr(status, &creds.error_path(object_path), &body));
    }
...
async fn do_gcs_put_acl(creds: &GcsCreds, object_path: &str, acl_json: &str) -> Result<(), PyErr> {
    ...
    headers.push(("content-type".to_string(), "application/json".to_string()));
    let (status, response) = do_request_light("PATCH", &url, &headers, Some(Bytes::from(body)), false)
        .await
        .map_err(PyRuntimeError::new_err)?;
    if !status.is_success() {
        return Err(status_to_pyerr(status, &creds.error_path(object_path), &response));
    }
    Ok(())
}
