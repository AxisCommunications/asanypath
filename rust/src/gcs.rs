...
impl GcsCreds {
    ...
    async fn auth_headers(&self) -> Vec<(String, String)> {
        ...
        // No explicit token: fall back to Application Default Credentials.
        // Skipped against emulators (STORAGE_EMULATOR_HOST), which need no auth.
        if std::env::var("STORAGE_EMULATOR_HOST").is_ok() {
            return vec![];
        }
        match adc_access_token().await {
            Ok(Some(token)) => bearer_headers(&token),
            Ok(None) => vec![],
            Err(err) => {
                eprintln!("asanypath: GCS ADC token acquisition failed: {err}");
                vec![]
            }
        }
    }
...
// ---------------------------------------------------------------------------
// Application Default Credentials (ADC) token minting
//
// When no explicit access token is supplied, resolve credentials the way the
// gcloud SDK does: GOOGLE_APPLICATION_CREDENTIALS, else the well-known ADC file
// written by `gcloud auth application-default login`. Minted tokens are cached
// in-process until shortly before expiry. Only the `authorized_user` grant
// (refresh-token flow, no crypto) is supported; service accounts still require
// an explicit token.
// ---------------------------------------------------------------------------

static ADC_TOKEN_CACHE: OnceLock<Mutex<HashMap<String, (String, Instant)>>> = OnceLock::new();

fn adc_cache() -> &'static Mutex<HashMap<String, (String, Instant)>> {
    ADC_TOKEN_CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}
...
fn adc_path() -> Option<String> {
    ...
    #[cfg(windows)]
    {
        base.push(std::env::var("APPDATA").ok()?);
    }
    #[cfg(not(windows))]
    {
        base.push(std::env::var("HOME").ok()?);
        base.push(".config");
    }
    base.push("gcloud");
    base.push("application_default_credentials.json");
    base.exists().then(|| base.to_string_lossy().into_owned())
}

async fn adc_access_token() -> Result<Option<String>, String> {
    let path = match adc_path() {
        Some(p) => p,
        None => return Ok(None),
    };
    if let Ok(guard) = adc_cache().lock() {
        if let Some((token, expiry)) = guard.get(&path) {
    ...
    let raw = match std::fs::read_to_string(&path) {
        Ok(r) => r,
        Err(_) => return Ok(None), // file vanished → stay lenient
    };
    let (token, ttl) = mint_adc_token(&raw)?.await?;
    if let Ok(mut guard) = adc_cache().lock() {
        let expiry = Instant::now() + ttl.saturating_sub(Duration::from_secs(60));
        guard.insert(path, (token.clone(), expiry));
    }
    Ok(Some(token))
}

/// Parse an ADC document and return a future that mints a token, or an error
/// for unsupported credential types.
fn mint_adc_token(
    raw: &str,
) -> Result<impl std::future::Future<Output = Result<(String, Duration), String>>, String> {
    let doc: sonic_rs::Value =
        sonic_rs::from_str(raw).map_err(|e| format!("ADC parse error: {e}"))?;
    match doc.get("type").and_then(|v| v.as_str()).unwrap_or("") {
        "authorized_user" => {
            let client_id = doc
                .get("client_id")
                .and_then(|v| v.as_str())
            ...
            if client_id.is_empty() || client_secret.is_empty() || refresh_token.is_empty() {
                return Err(
                    "ADC authorized_user missing client_id/client_secret/refresh_token".to_string(),
                );
            }
            Ok(mint_authorized_user(
                client_id,
                client_secret,
                refresh_token,
                token_uri,
        ...
        "service_account" => Err("GCS service-account ADC minting is not yet supported; run \
             `gcloud auth application-default login` or set GCS_ACCESS_TOKEN"
            .to_string()),
...
async fn mint_authorized_user(
    ...
    let (status, body) =
        do_request_light("POST", &token_uri, &headers, Some(Bytes::from(form)), false).await?;
    if !status.is_success() {
        return Err(format!(
            "ADC token endpoint returned {}: {}",
            status,
            String::from_utf8_lossy(&body)
        ));
    }
    let resp: sonic_rs::Value =
        sonic_rs::from_slice(&body).map_err(|e| format!("ADC token parse error: {e}"))?;
    let token = resp
        .get("access_token")
        .and_then(|v| v.as_str())
        .unwrap_or_default()
        .to_string();
    if token.is_empty() {
        return Err("ADC token response missing access_token".to_string());
    }
    let expires = resp.get("expires_in").and_then(|v| v.as_i64()).unwrap_or(3600);
    Ok((token, Duration::from_secs(expires.max(60) as u64)))
}
...
async fn do_gcs_is_dir(creds: &GcsCreds, prefix: &str, use_h2: bool) -> Result<bool, PyErr> {
    ...
    let headers = creds.auth_headers().await;
    let (status, body) = do_request_light("GET", &url, &headers, None, use_h2)
        .await
        .map_err(|e| PyRuntimeError::new_err(e))?;
    if !status.is_success() {
...
#[cfg(test)]
mod adc_tests {
    use super::mint_adc_token;

    #[test]
    fn rejects_service_account_adc() {
        let err = mint_adc_token(r#"{"type":"service_account","client_email":"x"}"#)
            .err()
            .expect("service_account should be rejected");
        assert!(err.contains("service-account"), "{err}");
    }

    #[test]
    fn rejects_unknown_adc_type() {
        let err = mint_adc_token(r#"{"type":"magic"}"#).err().unwrap();
        assert!(err.contains("unsupported"), "{err}");
    }

    #[test]
    fn rejects_authorized_user_missing_fields() {
        let err = mint_adc_token(r#"{"type":"authorized_user","client_id":"a"}"#)
            .err()
            .unwrap();
    ...
    #[test]
    fn accepts_complete_authorized_user() {
        let parsed = mint_adc_token(
