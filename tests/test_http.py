...
async def test_walk_recurses():
    ...
    with patch(f"{_HTTP_NATIVE}.http_scrape_links", new=AsyncMock(side_effect=fake_scrape)):
        triples = [
            (str(r), [str(x) for x in ds], [str(x) for x in fs]) async for r, ds, fs in p.walk()
        ]
    assert triples == [
        ("http://h/d", ["http://h/d/sub"], ["http://h/d/a.txt"]),
        ("http://h/d/sub", [], ["http://h/d/sub/b.txt"]),
    ]
...
async def test_integration_walk_python_server(http_server):
    ...
    assert sorted(str(f).rsplit("/", 1)[-1] for f in files) == ["a.txt", "b.log"]
    assert [str(d).rsplit("/", 1)[-1] for d in dirs] == ["sub"]
    _, _, sub_files = triples[1]
    assert [str(f).rsplit("/", 1)[-1] for f in sub_files] == ["c.txt"]
