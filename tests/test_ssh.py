...
class TestSSHPath(testbase):
    ...
    async def test_walk_topdown(self, sftp):
        ...
        assert [d.name for d in first_dirs] == ["sub"]
        assert [f.name for f in first_files] == ["f1"]
        # Second yield: sub with no dirs and one file.
        _, sub_dirs, sub_files = triples[1]
        assert sub_dirs == []
        assert [f.name for f in sub_files] == ["f2"]
...
async def test_walk_fast_path_uses_remote_find(sftp):
    ...
    assert [d.name for d in root_dirs] == ["subdir"]
    assert [f.name for f in root_files] == ["file.txt"]

    sub_path, sub_dirs, sub_files = triples[1]
    assert sub_dirs == []
    assert [f.name for f in sub_files] == ["f2.txt"]
    # SFTP was not used
...
async def test_walk_fast_path_bottom_up(sftp):
    ...
    assert [f.name for f in first_files] == ["f.txt"]
    second_base, second_dirs, _ = triples[1]
    assert second_base.name == "root"
    assert [d.name for d in second_dirs] == ["subdir"]
