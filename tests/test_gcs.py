...
class TestGCSPath(testbase):
    ...
    async def test_get_access_policy_decodes_object_acl(self, p, as_json):
        native_acl = {
        ...
        with patch("asanypath.gcs.gcs_get_acl", new=AsyncMock(return_value=response)) as mock:
    ...
    async def test_get_access_policy_rejects_malformed_native_acl(self, p):
        with patch("asanypath.gcs.gcs_get_acl", new=AsyncMock(return_value=[])):
