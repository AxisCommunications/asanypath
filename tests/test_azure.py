...
class TestAzurePath(testbase):
    ...
    async def test_get_access_policy_decodes_container_acl(self, p, as_json):
        native_acl = {
            "public_access": "blob",
            "signed_identifiers": [
        ...
        with patch(
            "asanypath.azure.az_get_container_acl", new=AsyncMock(return_value=response)
        ) as mock:
    ...
    async def test_get_access_policy_rejects_malformed_native_acl(self, p):
        with patch("asanypath.azure.az_get_container_acl", new=AsyncMock(return_value=[])):
    ...
    async def test_walk(self):
        ...
        assert "sub" in entries[0][1]
        assert "file.txt" in entries[0][2]
        assert any(str(e[0]) == "az://container/root/sub" for e in entries[1:])
        assert any("deep.txt" in e[2] for e in entries[1:])
...
def test_connection_string_populates_account_and_endpoint():
    cs = (
        "DefaultEndpointsProtocol=https;AccountName=acct;"
        "AccountKey=S0VZ==;EndpointSuffix=core.windows.net"
    )
    env = {"AZURE_STORAGE_CONNECTION_STRING": cs}
    with patch("asanypath.azure.getenv", side_effect=lambda k, d=None: env.get(k, d)):
        AzurePath._env_config = None
        try:
            p = AzurePath("az://container/blob.txt")
            assert p._account_name == "acct"
            assert p._account_key == "S0VZ=="
            assert p._endpoint_url == "https://acct.blob.core.windows.net"
        finally:
            AzurePath._env_config = None


def test_connection_string_blob_endpoint_and_sas():
    cs = (
        "BlobEndpoint=http://127.0.0.1:10000/devstoreaccount1;"
        "SharedAccessSignature=sv=2023-11-03&sig=abc"
    )
    env = {"AZURE_STORAGE_CONNECTION_STRING": cs}
    with patch("asanypath.azure.getenv", side_effect=lambda k, d=None: env.get(k, d)):
        AzurePath._env_config = None
        try:
            p = AzurePath("az://container/blob.txt")
            assert p._endpoint_url == "http://127.0.0.1:10000/devstoreaccount1"
            assert p._sas_token == "sv=2023-11-03&sig=abc"
        finally:
            AzurePath._env_config = None


def test_explicit_env_overrides_connection_string():
    env = {
        "AZURE_STORAGE_CONNECTION_STRING": "AccountName=fromcs;AccountKey=CS==",
        "AZURE_STORAGE_ACCOUNT": "fromenv",
    }
    with patch("asanypath.azure.getenv", side_effect=lambda k, d=None: env.get(k, d)):
        AzurePath._env_config = None
        try:
            p = AzurePath("az://container/blob.txt")
            assert p._account_name == "fromenv"
        finally:
            AzurePath._env_config = None
