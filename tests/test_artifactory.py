...
class TestArtifactoryPath(testbase):
    ...
    async def test_iterdir_does_not_resave_token_per_child(self, p, monkeypatch):
        """Children must inherit the token without hitting the keyring (perf bug)."""
        from asanypath import _credstore

        saves: list = []
        monkeypatch.setattr(_credstore, "save", lambda k, v: saves.append((k, v)))
        batcher = AsyncMock()
        batcher.list = AsyncMock(
            return_value=[("repo/a", False), ("repo/b", False), ("repo/c", False)]
        )
        with patch.object(p, "_get_batcher", return_value=batcher):
            children = [child async for child in p.iterdir()]
        assert len(children) == 3
        assert all(c._token == p._token for c in children)
        assert saves == []
...
def test_artifactory_loads_token_from_jfrog_cli_conf(tmp_path, monkeypatch):
    """No token/env/credstore → falls back to the JFrog CLI config."""
    import json

    from asanypath import _credstore

    conf = tmp_path / "jfrog-cli.conf.v6"
    conf.write_text(
        json.dumps(
            {
                "version": "6",
                "servers": [
                    {"url": "https://other.example/", "accessToken": "OTHER"},
                    {
                        "artifactoryUrl": "https://artifactory.example.com/artifactory/",
                        "accessToken": "CONFTOKEN",
                        "isDefault": True,
                    },
                ],
            }
    ...
    monkeypatch.setattr(_credstore, "load", lambda key, **_: None)
    with patch.dict("os.environ", {"JFROG_CLI_HOME": str(tmp_path)}, clear=True):
        ArtifactoryPath._env_config = None
        try:
            p = ArtifactoryPath(art_path())
