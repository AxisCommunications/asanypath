...
Covers S3, Azure Blob, GCS, Artifactory, and local filesystem backends.
Operations: write, read, exists, iterdir, unlink, cp (cloud: native
...
class AsAnyPathAdapter:
    ...
    async def write(self, uri: str, data: bytes) -> None:
        ...
        if uri.startswith("file://"):
            Path(uri.replace("file://", "", 1)).parent.mkdir(parents=True, exist_ok=True)
        await AsAnyPath(uri).write_bytes(data)

    async def read(self, uri: str) -> bytes:
        from asanypath import AsAnyPath

        return await AsAnyPath(uri).read_bytes()

    async def exists(self, uri: str) -> bool:
        from asanypath import AsAnyPath

        return await AsAnyPath(uri).exists()

    async def iterdir(self, uri: str) -> list[str]:
        from asanypath import AsAnyPath

        return [str(p) async for p in AsAnyPath(uri).iterdir()]

    async def unlink(self, uri: str) -> None:
        from asanypath import AsAnyPath

        await AsAnyPath(uri).unlink()
...
def _get_adapters(backend: str) -> list[tuple[str, list]]:
    ...
    if not backends:
        raise ValueError(f"Unknown backend: {backend}. Use: local, s3, az, gs, art, or all")
...
async def main() -> None:
    ...
    parser.add_argument(
        "--backend",
        default="local",
        help="Backend to benchmark: local, s3, az, gs, art, or all (default: local)",
    )
