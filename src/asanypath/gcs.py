...
Credentials: an explicit ``access_token`` or ``GCS_ACCESS_TOKEN`` is used when
present. Otherwise the native layer falls back to Application Default
Credentials (``GOOGLE_APPLICATION_CREDENTIALS`` or the well-known file written
by ``gcloud auth application-default login``), minting and caching an access
token. ADC is skipped when ``STORAGE_EMULATOR_HOST`` is set.
"""
