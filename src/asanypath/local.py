            ...
            try:
                with target_path.open("wb") as dst_file:
                    if chunk_size and getattr(src, "_supports_range_read", False):
                        size = getattr(await src.stat(), "st_size", None)
                        if not isinstance(size, int):
                            raise NotImplementedError("chunk_size requires a known source size")
                        for start in range(0, size, chunk_size):
                            chunk = await src._range_read(start, min(start + chunk_size, size))
